...
def filter_out_isolate_normalize_feature(edge_index, features, label, role = None, shrink_ratio = None, rnd_seed = 30):
    """
    ...
    print("Without shrinking, unique connected edges type: {}; shape: {}".format(type(connected_edges), connected_edges.shape))
    ...
    #     print(edge_index.shape, type(edge_index))
        print('excluded nodes number is: ', features.shape[0] - len(connected_nodes_idx))
    ...
    #     print(connect_label.shape, type(connect_label))
        ...
        if role is not None:
            ref_nodes_idx = set(connected_nodes_idx)
            for key, val in role.items():
                role[key] = sorted(mapper[idx] for idx in (set(val) & ref_nodes_idx) )

        return connect_edge_index, connect_features, connect_label, role
    else:
        print(' 1) Did not shrink the orignal graph or 2) there is no isolated node if shrink_ratio is None ')
        ...
        return connected_edges, features, label, role
