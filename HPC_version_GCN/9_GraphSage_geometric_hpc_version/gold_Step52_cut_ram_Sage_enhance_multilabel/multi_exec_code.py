...
def execute_investigate_validation(image_path, mini_batch_folder, tune_param_name, tune_val_label, tune_val, trainer_id = 0, model_epoch = [1]):
    ...
    model_eval_file_name = "{}model_eval.pkl".format(mini_batch_folder)
    with open(model_eval_file_name, "rb") as fp:
        validation_model = pickle.load(fp)
    
    for validation_epoch in model_epoch:
        tune_model_file_name = '{}model_epoch_{}.pkl'.format(tune_model_folder, validation_epoch)
        
        validation_model.load_state_dict(torch.load(tune_model_file_name, map_location=lambda storage, loc: storage))
...
def execute_test_tuning(image_path, mini_batch_folder, snapshot_epoch_list,
    ...
    best_model_file = "{}model_epoch_{}.pkl".format(tune_model_folder, ep_best)
    ...
    best_model.load_state_dict(torch.load(best_model_file, map_location=lambda storage, loc: storage))
    
    f1, accuracy = Cluster_investigate_evaluation(best_model, mini_batch_folder, eval_target = 'test_batch/test_batch_whole')
...
def step0_generate_clustering_machine(data_name, data, dataset, intermediate_data_path, datapath = None, train_batch_num = 2):
    
    print('Start to generate the clustering with {} train batches'.format(train_batch_num) )

    # set the basic settings for the future batches generation
    set_clustering_machine(data_name, data, dataset, intermediate_data_path, datapath = datapath, 
