...
def set_clustering_machine_validation_batch(intermediate_data_folder, sample_num = 10, \
...
#     check_folder_exist(intermediate_data_folder)  # if exist then delete
    ...
    os.makedirs(os.path.dirname(info_folder), exist_ok=True)
    t2 = time.time()
    clustering_machine.mini_batch_validation_clustering(intermediate_data_folder, sample_num = sample_num, \
                                                   batch_range = batch_range, info_folder = info_folder, info_file = info_file)
    validation_batch_production_time = time.time() - t2
    ...
    # output the memory usage information
    info_GPU_memory_folder = intermediate_data_folder + 'info_GPU_memory/'
    output_GPU_memory_usage('GPU_cost_setting_validation_batch.txt', info_GPU_memory_folder, comment ='after generating validation batches: ')
...
def Cluster_test_batch_run(trainer_id, intermediate_data_folder):
...
#     test_F1, test_accuracy = gcn_trainer.batch_validate(valid_batch_num = valid_part_num)
    ...
    info_GPU_memory_folder = intermediate_data_folder + 'info_GPU_memory/'
    output_GPU_memory_usage('Memory_use_run_cpu_test.txt', info_GPU_memory_folder, comment ='after testing minibatches: ')
...
def Cluster_tune_train_run(intermediate_data_folder, input_layer = [16, 16], epochs=300, \
    ...
    gcn_trainer.train(epoch_num=epochs, learning_rate = lr, weight_decay = weight_decay, \
                      mini_epoch_num = mini_epoch_num, train_batch_num = train_batch_num)
    train_period = time.time() - t1
    print('Training costs a total of {0:.4f} seconds!'.format(train_period))
    print('-' * 80)
...
def Cluster_investigate_validation(validation_model, mini_batch_folder):
    ...
    batch_file_name = mini_batch_folder + 'test/batch_whole'
    ...
    prediction = validation_model(test_edges, test_features)
