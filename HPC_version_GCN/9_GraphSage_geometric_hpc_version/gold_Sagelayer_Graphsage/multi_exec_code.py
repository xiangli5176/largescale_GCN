...
def execute_investigate(image_path, mini_batch_folder, tune_param_name, tune_val, trainer_id = 0, input_layer = [32], epoch_num = 300, \
    ...
    Train_peroid_f1, Train_peroid_accuracy = Cluster_train_valid_batch_investigate(mini_batch_folder, input_layer = input_layer, epochs=epoch_num, \
                                        dropout = dropout, lr = lr, weight_decay = weight_decay, mini_epoch_num = tune_val, output_period = output_period, \
                                                                train_part_num = train_part_num)
    
    # store the resulting data on the disk
    test_res_folder = image_path + 'tune_' + tune_param_name + '_' + str(tune_val) + '/'
    os.makedirs(os.path.dirname(test_res_folder), exist_ok=True)
    test_res_file = test_res_folder + 'res_trainer_' + str(trainer_id)
...
def step1_generate_train_batch(intermediate_data_path, sample_num = 10, \
    ...
    set_clustering_machine_train_batch(intermediate_data_path, sample_num = sample_num, batch_range = batch_range, info_folder = info_folder, info_file = info_file)
