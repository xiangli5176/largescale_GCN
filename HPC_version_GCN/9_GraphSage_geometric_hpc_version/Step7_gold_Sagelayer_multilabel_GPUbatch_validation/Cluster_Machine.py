...
class ClusteringMachine(object):
    ...
    def split_whole_nodes_edges_then_cluster(self, validation_ratio, test_ratio, train_batch_num = 2, validation_batch_num = 2):
        """
            Only split nodes
        ...
        train_nodes_global, validation_nodes_global = train_test_split(model_nodes_global, test_size = relative_validation_ratio)
        
        self.sg_train_nodes_global = self.random_clustering(train_nodes_global, train_batch_num)
        self.sg_validation_nodes_global = self.random_clustering(validation_nodes_global, validation_batch_num)
...
#         check_folder_exist(batch_file_folder)
    ...
    def mini_batch_validation_clustering(self, batch_folder, sample_num = 10, batch_range = (0, 1), info_folder = './info/', info_file = 'validation_batch_size_info.csv'):
        data_type = 'validation'
        batch_file_folder = batch_folder + data_type + '/'
#         check_folder_exist(batch_file_folder)
        os.makedirs(os.path.dirname(batch_file_folder), exist_ok=True)
        
        self.info_validation_batch_node_size, self.info_validation_batch_edge_size  = self.mini_batch_generate(batch_file_folder, self.sg_validation_nodes_global, sample_num = sample_num, batch_range = batch_range)
