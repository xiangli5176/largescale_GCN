...
def execute_investigate(image_path, mini_batch_folder, tune_param_name, tune_val_label, tune_val, trainer_id = 0, input_layer = [32], epoch_num = 300, \
                        dropout = 0.3, lr = 0.0001, weight_decay = 0.01, mini_epoch_num = 20, output_period = 10, \
                         train_part_num = 2):
    """
        return all test-F1 and validation-accuracy
    """
    Train_peroid_f1, Train_peroid_accuracy = Cluster_train_test_batch_investigate(mini_batch_folder, input_layer = input_layer, epochs=epoch_num, \
...
def step31_generate_validation_batch(intermediate_data_path, sample_num = 10, \
                               batch_range = (0, 1), info_folder = 'info/'):            
    # set the save path
    print('Start running for validation batch num: ' + str(batch_range) )
    info_file = 'validation_batch_size_info_{}.csv'.format(str(batch_range))

    # generate the train batches
    set_clustering_machine_validation_batch(intermediate_data_path, sample_num = sample_num, \
                                       batch_range = batch_range, info_folder = info_folder, info_file = info_file)

def step32_run_investigation_train_batch(image_data_path, intermediate_data_path, tune_param_name, tune_val_label, tune_val, train_batch_num, net_layer_num, GCN_layer, \
