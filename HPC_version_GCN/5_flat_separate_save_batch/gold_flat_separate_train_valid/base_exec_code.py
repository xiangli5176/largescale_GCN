...
def step0_generate_clustering_machine(data, intermediate_data_path, partition_nums, layers):            
...
def step1_generate_train_batch(intermediate_data_path, partition_nums, layers, train_frac = 1.0, train_part_num = 2):            
    for partn in partition_nums:
        for GCN_layer in layers:
            net_layer = len(GCN_layer) + 1
            hop_layer = net_layer
            ...
            # set the batch for train
            set_clustering_machine_train_batch(intermediate_data_folder, neigh_layer = hop_layer, train_frac = train_frac, train_part_num = partn)

    
def step2_generate_validation_batch(intermediate_data_path, partition_nums, layers, validation_frac = 1.0, valid_part_num = 2):            
    for partn in partition_nums:
        for GCN_layer in layers:
            net_layer = len(GCN_layer) + 1
            hop_layer = net_layer
            ...
            set_clustering_machine_validation_batch(intermediate_data_folder, neigh_layer = hop_layer, validation_frac = validation_frac, valid_part_num = valid_part_num)
...
if __name__ == '__main__':
    ...
    step1_generate_train_batch(data, data_name, dataset, image_data_path, intermediate_data_folder, partition_nums, layers)

    step2_generate_validation_batch(data, data_name, dataset, image_data_path, intermediate_data_folder, partition_nums, layers, valid_part_num = 2)
