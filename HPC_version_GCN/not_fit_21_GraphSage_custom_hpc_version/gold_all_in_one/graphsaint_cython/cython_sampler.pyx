...
cdef class Sampler:
    ...
    # p here is the index of the processor?
    ...
    cdef void sample(self, int p) nogil:
        ...
        while r < self.num_sample_per_proc:
            ...
            while i < self.size_subg:
                ...
                # each time sampled from all the train_neighbor list
...
cdef class FullBatch(Sampler):
    ...
    cdef void sample(self, int p) nogil:
        ...
        while r < self.num_sample_per_proc:
            idx_subg = p*self.num_sample_per_proc+r
            i = 0
            # put each train nodes one-by-one into the sampled vector
            ...
            # get rid of duplicated nodes inside the sample:
            sort(self.node_sampled[idx_subg].begin(),self.node_sampled[idx_subg].end())

            # unique: Removes all but the first element from every consecutive group of equivalent elements in the range [first,last)
            # with all  the removed element values replaced as "?" at the end, therefore we further use erase to get rid of them
            # return : An iterator to the element that follows the last element not removed.

            # this is to keep all the unique elements
            self.node_sampled[idx_subg].erase( unique(self.node_sampled[idx_subg].begin(), self.node_sampled[idx_subg].end()), self.node_sampled[idx_subg].end() )
...
cdef class Sage_Node(Sampler):
    ...
    cdef void sample(self, int p) nogil:
        ...
        while r < self.num_sample_per_proc:
            ...
            i = 0
            while i < self.size_subg:
                # go through each train node and add each's neighbor's 
                
                # for each node, sampled from its own 1, 2, 3 most adjacent neighbor
                sample = rand() % rand_range
                # lower_bound: Returns an iterator pointing to the first element in the range [first,last) which does not compare less than val.
