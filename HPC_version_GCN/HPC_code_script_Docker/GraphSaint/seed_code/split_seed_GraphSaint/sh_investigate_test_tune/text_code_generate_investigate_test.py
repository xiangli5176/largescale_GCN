...
if __name__ == '__main__':
    ...
    trainer_ids = list(range(3))  # for each 0 id, actually it executes for multi id
    # source path to copy from
    src_data_folder = datapath + "tuning_test_origin/"
    src_file_py = src_data_folder + "formal_run_test_id_0.py"

    src_file_pbs = src_data_folder + "step4_test_run_id_0.sh"
    ...
    for tune_val_label in tune_val_label_list:
        ...
        for trainer_id in trainer_ids:
            dst_file_code_name = "formal_run_test_id_" + str(trainer_id) + ".py"
            dst_file_py = tune_val_folder + dst_file_code_name
            generate_src_file(dst_file_py, src_file_py, tune_val_label, trainer_id)
            dst_file_pbs = tune_val_folder + "step4_test_run_id_" + str(trainer_id) + ".sh"
