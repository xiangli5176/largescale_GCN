...
def generate_src_file(dst_file, src_file, tune_val_label, trainer_id):
    with open(src_file, "r") as readin:
        with open(dst_file, "w") as writeout:
            for line in readin:
                if line.strip().startswith("tune_val_label_list"):
                    line = " " * 4 + "tune_val_label_list = [" + str(tune_val_label) + "]\n"
...
if __name__ == '__main__':
    ...
    # tune_val_label_list = [1, 2, 3]
    # trainer_ids = list(range(7))

    tune_val_label_list = [4]
    trainer_ids = list(range(7))

    # source path to copy from
    src_data_folder = datapath + "tuning_train_origin/"
    src_file_py = src_data_folder + "formal_run_train_id_0.py"
    src_file_pbs = src_data_folder + "step3_train_run_id_0.sh"
    ...
    for tune_val_label in tune_val_label_list:
        ...
        for trainer_id in trainer_ids:
            dst_file_code_name = "formal_run_train_id_" + str(trainer_id) + ".py"
            dst_file_py = tune_val_folder + dst_file_code_name
            generate_src_file(dst_file_py, src_file_py, tune_val_label, trainer_id)
            dst_file_pbs = tune_val_folder + "step3_train_run_id_" + str(trainer_id) + ".sh"
