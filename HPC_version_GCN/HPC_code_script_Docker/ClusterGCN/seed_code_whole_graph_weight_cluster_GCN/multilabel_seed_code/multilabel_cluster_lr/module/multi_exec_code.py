...
def execute_tuning_train(mini_batch_folder, tune_param_name, tune_val_label, tune_val, trainer_id = 0, input_layer = [32], epoch_num = 400,
                  dropout = 0.1, lr = 0.0001, weight_decay = 0.1, mini_epoch_num = 20, improved = False, diag_lambda = -1,
                  train_batch_num = 2):
    """
...
if __name__ == '__main__':
    ...
    res_model_epoch_list = list(range(20, 601, 20))
    ...
    step0_generate_clustering_machine(data, dataset, intermediate_data_folder, origin_train_batch_num, validation_ratio = 0.12, test_ratio = 0.22,
                                  mini_cluster_num = 64, round_num = round_num, improved = False, diag_lambda = 0.5)
    ...
    for tune_val_label, tune_val in zip(tune_val_label_list, tune_val_list):
        for trainer_id in trainer_list:
            step30_run_tune_train_batch(intermediate_data_folder, tune_param_name, tune_val_label, tune_val, train_batch_num, GCN_layer, \
                trainer_id = trainer_id, dropout = 0.1, lr = 0.001, weight_decay = 0.1, mini_epoch_num = 20, improved = False, diag_lambda = 0.5, epoch_num = 600)
        ...
        for tainer_id in trainer_list:
            step31_run_investigation_train_batch(intermediate_data_folder, tune_param_name, tune_val_label, tune_val, train_batch_num, net_layer_num, GCN_layer,
                        trainer_id = tainer_id, dropout = 0.1, lr = 0.0001, weight_decay = 0.01, mini_epoch_num = 10, improved = False, diag_lambda = -1,
                                             epoch_num = 600, output_period = 20)
