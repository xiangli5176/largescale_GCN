    ...
    def forward(self, edge_index, features, edge_weight=None):
        if len(self.layers) > 1:
            for i in range(len(self.layers)-1):
                features = self.layers[i](features, edge_index, edge_weight = edge_weight, dropout_training=self.training)
#                 features = F.dropout(features, p = self.dropout, training = self.training)
                features = self.bn_layers[i](features)
                features = F.relu(features)
                    
            features = self.layers[len(self.layers)-1](features, edge_index, edge_weight = edge_weight, dropout_training=self.training)
        else:
            features = self.layers[0](features, edge_index, edge_weight = edge_weight, dropout_training=self.training)    # for a single layer case
