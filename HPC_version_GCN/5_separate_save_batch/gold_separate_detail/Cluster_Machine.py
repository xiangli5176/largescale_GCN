...
class ClusteringMachine(object):
    ...
    def get_edge_weight(self, edge_index, num_nodes, edge_weight=None, improved=False, dtype=None):
        ...
        return {(edge_index[i][0], edge_index[i][1]) : normalized_edge_weight[i] for i in range(num_edge)}
...
#         cluster_membership = {node : i for i, node_list in enumerate(partition_list) for node in node_list}
    ...
    def mini_batch_generate(self, batch_file_folder, target_seed, k, fraction = 1.0, data_type = 'train'):
        ...
        edge_weight_global_dict = self.get_edge_weight(self.edge_index, self.node_count)
        ...
        for cluster in target_seed.keys():
            ...
            mini_edge_weight_local = \
                            [ edge_weight_global_dict[(edge[0], edge[1])] for edge in mini_edges_global ] + \
                            [ edge_weight_global_dict[(edge[1], edge[0])] for edge in mini_edges_global ] + \
                            [ edge_weight_global_dict[(i, i)] for i in mini_nodes_global ]
