...
cimport graphsaint_cython.cython_utils as cutils
import graphsaint_cython.cython_utils as cutils
...
cdef class Sage_Node(Sampler):
    cdef int size_subg
    cdef vector[int] neigh_train_indptr
    cdef vector[int] neigh_train_data

    def __cinit__(self, np.ndarray[int,ndim=1,mode='c'] adj_indptr,
                        np.ndarray[int,ndim=1,mode='c'] adj_indices,
                        np.ndarray[int,ndim=1,mode='c'] node_train,
                        int num_proc, int num_sample_per_proc,
                        np.ndarray[int,ndim=1,mode='c'] neigh_train_indptr,
                        np.ndarray[int,ndim=1,mode='c'] neigh_train_data,
                        int size_subg):

        self.size_subg = size_subg

        cutils.npy2vec_int(neigh_train_indptr, self.neigh_train_indptr)
        cutils.npy2vec_int(neigh_train_data, self.neigh_train_data)
    
    cdef void sample(self, int p) nogil:

        cdef int i = 0
        cdef int r = 0
        cdef int train_idx = 0

        cdef int idx_subg
        cdef int sample_train_id
        cdef int num_neighbors
        cdef int rand_range = self.node_train_vec.size()
        while r < self.num_sample_per_proc:
            ...
            while i < self.size_subg:
                ...
                # sample_train_id = rand()%rand_range
                train_idx = train_idx % rand_range
                sample_train_id = self.node_train_vec[train_idx]
                num_neighbors = self.neigh_train_indptr[sample_train_id+1] - self.neigh_train_indptr[sample_train_id]

                self.node_sampled[idx_subg].reserve(self.node_sampled[idx_subg].size() + num_neighbors)
                self.node_sampled[idx_subg].insert(self.node_sampled[idx_subg].end(), self.neigh_train_data.begin() + self.neigh_train_indptr[sample_train_id], self.neigh_train_data.begin() + self.neigh_train_indptr[sample_train_id+1])
