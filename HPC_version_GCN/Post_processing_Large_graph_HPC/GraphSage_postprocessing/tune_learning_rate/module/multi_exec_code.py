...
def execute_tuning_train(mini_batch_folder, tune_param_name, tune_val_label, trainer_id = 0, input_layer = [32], epoch_num = 400, \
...
def execute_tuning_validation(image_path, mini_batch_folder, tune_param_name, tune_val_label, trainer_id = 0):
...
def execute_investigate(image_path, mini_batch_folder, tune_param_name, tune_val_label, trainer_id = 0, input_layer = [32], epoch_num = 300, \
...
def step0_generate_clustering_machine(data, dataset, intermediate_data_path, train_batch_num, \
                                        test_ratio = 0.05, validation_ratio = 0.85, mini_cluster_num = 16, round_num = 2):            
    print('Start running for train batch num: ' + str(train_batch_num) )

    # set the basic settings for the future batches generation
    set_clustering_machine(data, dataset, intermediate_data_path, test_ratio = test_ratio, validation_ratio = validation_ratio, \
...
def step40_run_tune_validation_whole(image_data_path, intermediate_data_path, tune_param_name, tune_val_label, train_batch_num, net_layer_num, \
                    trainer_id = 0): 
    # set the save path
    print('Start running training for partition num: ' + str(train_batch_num) )
    ...
    execute_tuning_validation(img_path, intermediate_data_path, tune_param_name, tune_val_label, trainer_id = trainer_id)

            
def step31_run_investigation_train_batch(image_data_path, intermediate_data_path, tune_param_name, tune_val, train_batch_num, net_layer_num, GCN_layer, \
    ...
    execute_investigate(img_path, intermediate_data_path, tune_param_name, tune_val, trainer_id = trainer_id, input_layer = GCN_layer, epoch_num = epoch_num, \
...
if __name__ == '__main__':
    ...
    # dataset = Reddit(root = remote_data_root + '/' + data_name)
    # data = dataset[0]
    ...
    step0_generate_clustering_machine(data, dataset, intermediate_data_folder, origin_train_batch_num, test_ratio = 0.05, validation_ratio = 0.85, mini_cluster_num = 32, round_num = 2)
    ...
    #     for trainer_id in trainer_list:
    #         step30_run_tune_train_batch(intermediate_data_folder, tune_param_name, tune_val_label, train_batch_num, GCN_layer, \
    ...
    # step50_run_tune_summarize_whole(data_name, image_data_path, intermediate_data_folder, tune_param_name, tune_val_label_list, \
    #                                 train_batch_num, net_layer_num, trainer_list)

    # train-batch investigate
    for tune_val in tune_val_list:
        for tainer_id in trainer_list:
            step31_run_investigation_train_batch(image_data_path, intermediate_data_folder, tune_param_name, tune_val, train_batch_num, net_layer_num, GCN_layer, \
                            trainer_id = tainer_id, dropout = 0.1, lr = 0.0001, weight_decay = 0.1, mini_epoch_num = 20, epoch_num = 400, output_period = 40)

    for tune_val in tune_val_list:
        step41_run_investigation_summarize_whole(data_name, image_data_path, intermediate_data_folder, tune_param_name, tune_val, \
                                        train_batch_num, net_layer_num, trainer_list)