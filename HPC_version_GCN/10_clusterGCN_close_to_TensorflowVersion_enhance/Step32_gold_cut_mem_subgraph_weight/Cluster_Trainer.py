...
#         ave_loss = torch.nn.functional.nll_loss(predictions[tr_train_nodes], tr_target[tr_train_nodes])
    ...
    def train_investigate_F1(self, tune_model_folder, epoch_num=10, learning_rate=0.01, weight_decay = 0.01, mini_epoch_num = 1, output_period = 10, train_batch_num = 2):
        """
            *** Periodically output the F1 score during training for each tuning values. After certain number of epochs ***
            tune_model_folder : save the snapshots under a folder specific to a tuning parameter value
...
#             For test purpose, we let the clusters to follow specific order
            ...
            for cluster in train_clusters:
                ...
                train_batch_file_name = "{}train_batch/batch_{}".format(self.data_folder, cluster)
                ...
                for mini_epoch in range(mini_epoch_num):
                    ...
                    if real_epoch_num % output_period == 0:
                        ...
                        tune_model_file_name = "{}model_epoch_{}.pkl".format(tune_model_folder, real_epoch_num)
                        
                        torch.save(self.model.state_dict(), tune_model_file_name)  # store the current state_dict() into the file: 'tmp.pkl'
...
#             For test purpose, we let the clusters to follow specific order
            ...
            for cluster in train_clusters:
                # read in the train data from the pickle files
                batch_file_name = "{}train_batch/batch_{}".format(self.data_folder, cluster)
    ...
    def whole_cpu_test(self):
        ...
        batch_file_name = "{}test_batch/test_batch_whole".format(self.data_folder)
