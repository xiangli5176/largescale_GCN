...
#     clustering_machine.split_cluster_nodes_edges(test_ratio, validation_ratio, partition_num = train_batch_num)
    ...
    t1 = time.time()
    clustering_machine.split_cluster_nodes_edges(test_ratio, validation_ratio, train_batch_num = train_batch_num)
...
def Cluster_train_batch_run(trainer_id, intermediate_data_folder, input_layer = [16, 16], epochs=300, \
                           dropout = 0.3, lr = 0.01, weight_decay = 0.01, mini_epoch_num = 5, select_group_size = 1, \
    ...
    gcn_trainer.train(epoch_num=epochs, learning_rate=lr, weight_decay=weight_decay, mini_epoch_num = mini_epoch_num, \
                      train_batch_num = train_batch_num, select_group_size = select_group_size)
...
def Cluster_tune_train_run(intermediate_data_folder, input_layer = [16, 16], epochs=300, \
    ...
    gcn_trainer.train(epoch_num=epochs, learning_rate=lr, weight_decay=weight_decay, \
                       mini_epoch_num = mini_epoch_num, train_batch_num = train_batch_num, select_group_size = select_group_size)
    train_period = time.time() - t1
    print('Training costs a total of {0:.4f} seconds!'.format(train_period))
    print('-' * 80)
