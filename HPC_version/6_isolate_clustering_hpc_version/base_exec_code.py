...
def set_clustering_machine(data, image_path, intermediate_data_folder, test_ratio = 0.05, validation_ratio = 0.85, train_part_num = 2, mini_cluster_num = 16, round_num = 2):
    """
        Set the batch machine plus generate the training batches
            ...
            2) image_path:  result path for image to be stored
            3) intermediate_data_folder: path to store the intermediate generated data
            4) test_ratio, validation_ratio: data split ratio
            5) train_part_num:  batch number to be generated for training
            6) mini_cluster_num:  number of mini-clusters to be recombined later
            7) round_num:  number of round for shuffling and generating the batches
    """
...
def step0_generate_clustering_machine(data, image_data_path, intermediate_data_path, partition_nums, layers, mini_cluster_num = 16, round_num = 2):            
    for partn in partition_nums:
        for GCN_layer in layers:
            ...
            set_clustering_machine(data, img_path, intermediate_data_folder, test_ratio = 0.05, validation_ratio = 0.85, \
                                train_part_num = partn, mini_cluster_num = mini_cluster_num, round_num = round_num)

def step1_generate_train_batch(image_data_path, intermediate_data_path, partition_nums, layers):            
    for partn in partition_nums:
        for GCN_layer in layers:
            net_layer = len(GCN_layer) + 1
            hop_layer = net_layer
...
def step2_generate_validation_batch(image_data_path, intermediate_data_path, partition_nums, layers):            
    for partn in partition_nums:
        for GCN_layer in layers:
            net_layer = len(GCN_layer) + 1
            hop_layer = net_layer
...
if __name__ == '__main__':
    ...
    step0_generate_clustering_machine(data, image_data_path, intermediate_data_folder, partition_nums, layers, mini_cluster_num = 16, round_num = 2)

    step1_generate_train_batch(image_data_path, intermediate_data_folder, partition_nums, layers)

    step2_generate_validation_batch(image_data_path, intermediate_data_folder, partition_nums, layers)
