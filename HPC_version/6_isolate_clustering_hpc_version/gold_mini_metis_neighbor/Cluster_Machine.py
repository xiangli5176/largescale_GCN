...
class ClusteringMachine(object):
    ...
    def split_cluster_nodes_edges(self, test_ratio, validation_ratio, mini_cluster_num = 4, train_batch_num = 2, round_num = 2):
        """
        ...
        # we may have some extra benifits from this strategy: 1) use metis to divide the graph into mini-clusters; 2) further split train-test in each cluster
        ...
        # wish to use a self.sg_batch_nodes_global directly generated from all the self.sg_train_nodes_global
        self.sg_batch_nodes_global = {}
        ...
        for round_id in range(round_num):
            ...
            for local_batch_id, group in enumerate(combine_group):
                ...
                self.sg_batch_nodes_global[global_batch_id] = list(chain.from_iterable(mini_cluster_nodes_global[cluster_id] for cluster_id in group))
                sg_validation_nodes_global[global_batch_id] = list(chain.from_iterable(mini_cluster_validation_nodes_global[cluster_id] for cluster_id in group))
        ...
        for batch in self.sg_batch_nodes_global.keys():
            # record the information of each recombined batch:
            self.info_isolate_cluster_size[batch] = len(self.sg_batch_nodes_global[batch])
