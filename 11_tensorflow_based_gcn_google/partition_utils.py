...
def partition_graph(adj, idx_nodes, num_clusters):
  """partition a graph by METIS.
    As used in the main:   the inputs are: train_adj, train_data , number of clusters
  """
  ...
  train_adj_lil = adj[idx_nodes, :][:, idx_nodes].tolil()   # Convert this matrix to List of Lists format.  converted into <class 'scipy.sparse.lil.lil_matrix'>
  # this is exactly the same as :  train_adj_lil = adj.tolil()

  train_ord_map = dict()
  train_adj_lists = [[] for _ in range(num_nodes)]

  for i in range(num_nodes):
    rows = train_adj_lil[i].rows[0]   # extract the neighbors of the node i (list)
    ...
    train_ord_map[idx_nodes[i]] = i   # create mapping between global and local
  ...
  for nd_idx in range(num_nodes):
    ...
    parts[gp_idx].append(nd_orig_idx)   # append the global id of the node into the parts

    for nb_orig_idx in adj[nd_orig_idx].indices:
      nb_idx = train_ord_map[nb_orig_idx]   # get the local id of the neighbor node
  ...
  """
    parts:  a list of list containing all groups of node ids
  """
