...
from tensorflow.compat.v1 import gfile      #  tf.compat.v1.gfile : Import router for file_io.
...
def preprocess_multicluster(adj, parts, features, y_train, train_mask, num_clusters, block_size, diag_lambda=-1):
  """Generate the batch for multiple clusters.
    every block_size, we will append it into one batches
...
def preprocess(adj, features, y_train, train_mask, visible_data, num_clusters, diag_lambda=-1):
  """Do graph partitioning and preprocessing for SGD training.
    utils.preprocess(train_adj, train_feats, y_train, train_mask, visible_data, FLAGS.num_clusters, FLAGS.diag_lambda)
  """
...
"""
    parts (list of numpy arrays) :   global node id grouped by clusters
    features_batches (list of numpy arrays) : features for different clusters according to the parts
    support_batches:  (coords, values, shape) : edge weights in the tuple form
    y_train_batches (list of numpy arrays):   class labels
"""
...
def load_graphsage_data(dataset_path, dataset_str, normalize=True):
  ...
  # this is to create a better implementation of the id-label dict

  broken_count = 0
  to_remove = []
  for node in graph_nx.nodes():
    if node not in id_map:    # select specific nodes inside the map
      to_remove.append(node)
      broken_count += 1
  for node in to_remove:
    graph_nx.remove_node(node)
  tf.logging.info('Removed %d nodes that lacked proper annotations due to networkx versioning issues', broken_count)

  feats = np.load(gfile.Open('{}/{}/{}-feats.npy'.format(dataset_path, dataset_str, dataset_str), 'rb')).astype(np.float32)
  # load feats as a numpy array

  tf.logging.info('Loaded data (%f seconds).. now preprocessing..', time.time() - start_time)
  ...
  # generatre the node id: test validation and train in numpy array for node id
  ...
  # train_edges: a numpy array of tuples for edges
  ...
  def _construct_adj(edges):
    ...
    adj = sp.csr_matrix( ( np.ones((edges.shape[0]), dtype=np.float32), (edges[:, 0], edges[:, 1]) ), shape=(num_data, num_data) )
  ...
  """
    num_data (int) : number of graph nodes of concern
    train_adj (csr_matrix) : csr format of of the matrix ; print format :  edge, weight    ,   e.g.   (2, 3)  2
    full_adj  (csr_matrix) : csr format of the matrix for the whole graph
    feats  (numpy.array):   all the features
    train_feats (numpy.array) :   features of all the train nodes
    test_feats (numpy.array) :   features of all the test nodes
    labels (numpy.array):   class labels for all the nodes
    train_data (numpy.array):   node id for train data
    val_data (numpy.array)  :   node id for validation nodes
    test_data (numpy.array)  :  node id for test nodes
  """
